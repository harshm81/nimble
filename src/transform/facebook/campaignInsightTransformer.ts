...
import { CampaignInsightInput, extractAction } from '../../db/repositories/facebookRepo';

export function transformCampaignInsight(raw: FacebookCampaignInsightRaw, syncedAt: Date): CampaignInsightInput {
  return {
    campaignId:           raw.campaign_id   ?? '',
    campaignName:         raw.campaign_name ?? null,
    reportDate:           raw.date_start ? new Date(raw.date_start) : (() => { throw new Error('Facebook campaign insight missing required date_start'); })(),
